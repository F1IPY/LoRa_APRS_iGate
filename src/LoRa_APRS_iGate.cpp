...
void loop() {
    if (Config.digi.ecoMode == 1) {
        ...
        if (packet != "") {
            bool isAprsPacket = (packet.length() >= 3 && packet.substring(0, 3) == "\x3c\xff\x01");
            ...
            if (isAprsPacket) {
                ...
                if (Config.tnc.rnsActive) TNC_Utils::sendToRNSClients(packet);          // Forward raw Reticulum packet to RNS clients
            }
        }
